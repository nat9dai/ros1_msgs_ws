  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::mavros_msgs::Trajectory_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.point_valid.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint8_t>::stream(s, true ? std::string() : indent + "    ", v.point_valid[i]);
    }
    if (v.point_valid.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.command.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint16_t>::stream(s, true ? std::string() : indent + "    ", v.command[i]);
    }
    if (v.command.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "time_horizon: ";
    if (v.time_horizon.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.time_horizon.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.time_horizon[i]);
    }
    if (v.time_horizon.empty() || true)
      s << "]";
  }
