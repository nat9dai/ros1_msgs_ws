  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::mavros_msgs::Mavlink_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.payload64.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint64_t>::stream(s, true ? std::string() : indent + "    ", v.payload64[i]);
    }
    if (v.payload64.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.signature.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint8_t>::stream(s, true ? std::string() : indent + "    ", v.signature[i]);
    }
    if (v.signature.empty() || true)
      s << "]";
  }
