...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<ParamValue>)))
  ...
  (cl:format cl:nil "# Parameter value storage type.~%#~%# Integer and float fields:~%#~%# if integer != 0: it is integer value~%# else if real != 0.0: it is float value~%# else: it is zero.~%~%# XXX DEPRECATED: replaced by rmw_interfaces/ParameterValue~%~%int64 integer~%float64 real~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'ParamValue)))
  "Returns full string definition for message of type 'ParamValue"
  (cl:format cl:nil "# Parameter value storage type.~%#~%# Integer and float fields:~%#~%# if integer != 0: it is integer value~%# else if real != 0.0: it is float value~%# else: it is zero.~%~%# XXX DEPRECATED: replaced by rmw_interfaces/ParameterValue~%~%int64 integer~%float64 real~%~%~%"))
